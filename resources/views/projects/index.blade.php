                    ...
                    <div class="space-y-2">
                        <div class="flex items-start justify-between">
                            ...
                            <div class="flex space-x-1">
                                <a href="{{ route('projects.show', $project) }}" class="inline-flex items-center px-2 py-1 text-[11px] leading-4 font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500/50">
                        ...
                        <tr>
                            ...
                            <td class="px-4 py-2 whitespace-nowrap text-[11px] leading-4 font-medium">
                                <a href="{{ route('projects.show', $project) }}" 
                                   class="inline-flex items-center justify-center px-3 py-1.5 bg-blue-600 text-white font-medium rounded-lg shadow-md hover:shadow-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transition-all duration-200 transform hover:scale-105">
...
<div x-data="{ 
    ...
    deleteProject() {
        ...
        fetch(`/projects/${this.projectToDelete.id}`, {
            method: 'POST',
            body: formData,
            headers: {
                'X-Requested-With': 'XMLHttpRequest'
        ...
        .then(data => {
            if (data.success) {
                ...
                throw new Error(data.message || 'Terjadi kesalahan saat menghapus proyek');
            }
        })
        .catch(error => {
            console.error('Error:', error);
     ...
     @open-delete-modal.window="openDeleteModal($event.detail.project)"
     x-cloak x-show="showDeleteModal" 
     x-transition:enter="transition ease-out duration-300" 
     x-transition:enter-start="opacity-0" 
     x-transition:enter-end="opacity-100" 
     x-transition:leave="transition ease-in duration-200" 
    ...
    <!-- Background overlay -->
    <div class="fixed inset-0 bg-black/70 backdrop-blur-md" @click="closeDeleteModal()"></div>
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        <div x-show="showDeleteModal" 
             ...
             x-transition:leave-start="opacity-100 scale-100" 
             x-transition:leave-end="opacity-0 scale-95"
             class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-200/50 ring-1 ring-gray-100/50">
            ...
            <div x-show="showSuccess" 
                 ...
                 x-transition:enter-start="opacity-0 -translate-y-2" 
                 x-transition:enter-end="opacity-100 translate-y-0"
                 class="absolute top-4 left-4 right-4 z-50 bg-green-500 text-white px-4 py-3 rounded-xl shadow-lg flex items-center space-x-3">
                <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                </svg>
                <span class="font-medium">Proyek berhasil dihapus!</span>
            </div>
            ...
            <div class="p-6">
                <div class="text-center mb-6">
                    ...
                    <p class="text-gray-600 mb-0" x-text="projectToDelete ? `Anda akan menghapus proyek '${projectToDelete.website_name}' beserta semua data terkait.` : 'Anda akan menghapus proyek ini beserta semua data terkait.'"></p>
